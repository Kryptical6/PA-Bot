...
export function buildSessionEmbed(userId: string, logsToday: number, cfg: any, status: 'active' | 'done' = 'active'): EmbedBuilder {
  ...
  if (status === 'active') {
    embed
      ...
      .setDescription(`Your log session is running. Submit your logs as normal — the count below updates live.`)
      .addFields(
        { name: 'Logs submitted today', value: String(logsToday), inline: true },
    ...
    embed
      ...
      .setDescription('Your session has been submitted. Well done.')
...
export async function updateSessionDM(client: Client, userId: string): Promise<void> {
  try {
    ...
    const logsToday = (dayLog[0]?.submitted || 0) + (dayLog[0]?.approved || 0) + (dayLog[0]?.denied || 0);

    const cfg = (await sql`SELECT soft_target FROM spa_audit_config WHERE user_id = ${userId}`)[0] ?? { soft_target: 10 };
...
export async function checkExpiredSessions(client: Client): Promise<void> {
  ...
  for (const session of expired) {
    await closeSession(client, session, 'expired', 'This session was automatically closed after 12 hours of inactivity. Your session has been marked as missed.');

    // Mark today as incomplete in daily logs
    const today = new Date().toISOString().split('T')[0];
    await sql`
...
export async function postSessionSummary(client: Client, userId: string, sessionId: number, reviewType: string, reviewedItems: string[], postsReviewed: number): Promise<void> {
  const today = new Date().toISOString().split('T')[0];
  const dayLog = await sql`SELECT * FROM spa_daily_logs WHERE user_id = ${userId} AND log_date = ${today}`;
  const log = dayLog[0] ?? { submitted: 0, approved: 0, denied: 0 };

  const session = (await sql`SELECT * FROM spa_log_sessions WHERE id = ${sessionId}`)[0];
  const durationMs = new Date(session.completed_at ?? new Date()).getTime() - new Date(session.started_at).getTime();
  const hours   = Math.floor(durationMs / 3600000);
  const minutes = Math.floor((durationMs % 3600000) / 60000);

  // Format reviewed items as mentions
  const typeLabel: Record<string, string> = { sections: 'Sections', individuals: 'Individual PAs', mix: 'Mix' };
  const formattedItems = reviewedItems.map(id => {
    const cleaned = id.trim();
    if (/^\d{17,20}$/.test(cleaned)) {
      // Could be user ID or channel ID — we stored the type separately
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: 'Logs submitted',   value: `${log.submitted ?? 0} (approved: ${log.approved ?? 0}, denied: ${log.denied ?? 0})`, inline: false },
