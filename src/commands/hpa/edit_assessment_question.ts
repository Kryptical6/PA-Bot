...
export async function execute(i: ChatInputCommandInteraction): Promise<void> {
  ...
  await i.showModal({
    ...
    components: [
      ...
      { type: 1, components: [{ type: 4, customId: 'correct_reason', label: 'Expected Reason (shown in results)', style: 2, required: false, value: q.correct_reason ?? '' }] },
      { type: 1, components: [{ type: 4, customId: 'context', label: 'Context shown to user (optional)', style: 2, required: false, value: q.context ?? '' }] },
    ]
  });
  ...
  await sql`UPDATE assessment_questions SET post_id = ${postId}, correct_answer = ${answer}, correct_reason = ${correctReason}, context = ${context} WHERE id = ${qId}`;
