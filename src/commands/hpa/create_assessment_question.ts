...
export async function execute(i: ChatInputCommandInteraction): Promise<void> {
  ...
  await i.showModal({
    ...
    components: [
      ...
      { type: 1, components: [{ type: 4, customId: 'correct_reason', label: 'Expected Reason (shown in results)', style: 2, required: false, maxLength: 500, placeholder: 'e.g. Post contains graphic violence' }] },
      { type: 1, components: [{ type: 4, customId: 'context', label: 'Context shown to user (optional)', style: 2, required: false, maxLength: 500 }] },
    ]
  });
  ...
  const correctReason = modal.fields.getTextInputValue('correct_reason').trim() || null;
  ...
  const [result] = await sql`INSERT INTO assessment_questions (assessment_id, post_id, context, correct_answer, correct_reason, is_scripting) VALUES (${assessmentId}, ${postId}, ${context}, ${answer}, ${correctReason}, ${isScripting}) RETURNING id`;
  await modal.editReply({ embeds: [successEmbed('Question Added', `Q#${result.id} added to assessment ${assessmentId}.\nPost ID: \`${postId}\` | Correct: **${answer}**${correctReason ? `\nExpected reason: ${correctReason}` : ''}${isScripting ? ' | 📝 Scripting' : ''}`)] });
}
